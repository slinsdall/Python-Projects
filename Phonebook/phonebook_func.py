...
def center_window(self, w, h): # pass in the tkinter from (master) reference and the w and h
    ...
    centerGeo = self.mastergeometry('{}x{}+{}+{}'.format(w, h, x, y))
    return centerGeo
...
def create_db(self):
    ...
    with conn:
        ...
        #You must commit() to save changes & close database connection
        conn.commit()
    conn.close()
    first_run(self)
